...
AnimatedSprite::AnimatedSprite(const Time frameTime, const bool paused, const bool looped) :
...
void AnimatedSprite::setLooped(const bool looped)
{
	m_isLooped = looped;
}
...
{
	const IntRect rect = m_animation->getFrame(m_currentFrame);

	const float width = static_cast<float>(abs(rect.width));
	const float height = static_cast<float>(abs(rect.height));
...
void AnimatedSprite::setFrame(const size_t newFrame, const bool resetTime)
{
	if (m_animation)
	{
		//calculate new vertex positions and texture coordiantes
		const IntRect rect = m_animation->getFrame(newFrame);
